...
function domToReact(nodes, options) {
    ...
    var isOnDonePresent = typeof options.onDone === 'function';
    var replacement;
    var props;
    var children;

    if (isOnDonePresent)
        options.onDone = debounce(options.onDone, 50);
    ...
    for (var i = 0, len = nodes.length; i < len; i++) {
        ...
        if (isReplacePresent) {
            ...
            // if replacement is null or false  then skip element
        ...
        if (node.type === 'script' || node.type === 'style') {
            ...
            if (node.name === 'textarea' && node.children[0]) {
                ...
                children = domToReact(node.children, {replace: options.replace});
            }
        ...
        if (isOnDonePresent && i >= result.length)
            options.onDone();
        }
